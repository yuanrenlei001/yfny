App({
  data: {
    ...
    // 请求地址
    request_url: "https://www.yfnz2010.cn/",
  ...
  get_request_urls(a, c, plugins, params) {
    a = a || "index";
    c = c || "index";
  ...
  user_auth_login(object, method, auth_data) {
    ...
    wx.checkSession({
      success: function () {
        ...
        var datalogin = wx.getStorageSync(self.data.data_user_id) || null;
          wx.setStorage({
...
user_auth_vip_login(object, method, auth_data,name,password,reg) {
  var self = this;
  console.log(reg)
  ...
  wx.checkSession({
    success: function () {
        ...
        if (openid == null)
      {
        ...
        console.log(22)
        self.get_user_login_info(object, method, openid, auth_data,name,password,reg);
      }
      }else{
        if (openid == null)
      {
        ...
        console.log(22)
        self.get_user_login_info(object, method, openid, auth_data,name,password);
      }
      }
          ...
          if (res.code) {
            ...
            wx.request({
              ...
              success: (res) => {
                wx.hideLoading();
                console.log(res.data)
                  this.data.isOpenid = res.data.data.openid
                if (res.data.code == 0) {
   ...
   * auth_data  授权数据
   */
  get_user_login_info(object, method, openid, auth_data,name,password,reg) {
    // 邀请人参数
    ...
    if(reg){
      console.log('注册')
      wx.request({
        url: self.get_request_url('RegWeXin', 'user'),
        method: 'POST',
        data: {
          "weixin_openid": openid,
          "accounts": name,
          "pwd": password,
          'nickname':auth_data.userInfo.nickName,
          'gender':auth_data.userInfo.gender,
          'avatar':auth_data.userInfo.avatarUrl,
          'type':'register'
        },
        dataType: 'json',
        header: { 'content-type': 'application/x-www-form-urlencoded' },
        success: (res) => {
          ...
          if(res.data.code == 0){
            console.log(self.data.data_user_id)
            wx.getStorageSync('data_user_id');
            wx.setStorageSync('data_user_id',res.data.data.id )
  console.log(wx.getStorageSync('data_user_id'))
  if (typeof object === 'object' && (method || null) != null) {
    object[method]();
  }
          }else{
  ...
  console.log(referrer)
            ...
            var obj = {};
            obj['encrypted_data'] = auth_data.encryptedData;
            obj['iv'] = auth_data.iv;
            obj['openid'] = openid;
            self.data._data = obj
            wx.setStorageSync('_data', obj)
            wx.request({
  ...
  operation_event(e) {
    ...
    if (value != null) {
      switch (type) {
        ...
        case 1:
          // console.log(value)
          // if(value == 'https://www.yfnz2010.cn/index.php?s=car/index/index.html'){
