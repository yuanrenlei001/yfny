    ...
    if (user != false) {
      ...
      if (app.user_is_need_login(user)) {
        ...
        wx.request({
          ...
          method: 'POST',
          data: { "goods_id": this.data.goods.id, "stock": this.data.temp_buy_number, "spec": JSON.stringify(spec),'warehouse_new_id':this.data.address_Name.id },
          dataType: 'json',
          success: (res) => {
            wx.hideLoading();
            if (res.data.code == 0) {
